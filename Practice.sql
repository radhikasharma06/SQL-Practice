...
select age, loan , education from bank_details
...
select* from bank_details order by balance limit 1  #to find min balance
 select*from bank_details where balance in (select min(balance) from bank_details) #another approach to find min balance 
