...
select c.course_id , c.course_name , c.course_description, s.student_id, s.student_name , s.student_course_id from course c 
...
##cross join act a s an inner join if we give a condition.
...
EXPLAIN ANALYZE select * from course4 where  course_id = 106 or course_name = 'fsds' #EXPLAIN ANALYZE (in MySQL 8.0.18+ / some MariaDB versions) actually runs the query and prints the execution plan plus real stats, like:
		#which access method was used (index lookup, index merge, or full table scan)
		#how many rows it estimated vs how many it actually read/returned
		#timing per step

explain select * from course4 where  course_id = 106

analyze table course4 #ANALYZE TABLE course4; tells the database to recompute / refresh table statistics that the optimizer uses to choose good query plans.

describe course4 #DESCRIBE course4; (same as DESC course4;) shows the structure of the table — basically the schema/columns.
...
(select course_description , course_name from course 
union 
select student_id , student_name from student )
...
with outcoume_corss as (select c.course_id , c.course_name , c.course_description ,s.student_id,s.student_name ,s.student_course_id from course c
